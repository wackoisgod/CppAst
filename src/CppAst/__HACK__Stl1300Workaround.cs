...
namespace Biohazrd
{
    ...
    public struct SourceLocation
    {
        ...
        public string ToString(bool includeColumn)
        {
            ...
            if (Line == 0) { return sourceFile; }

            if (Column != 0 && includeColumn) { return $"{sourceFile}:{Line}:{Column}"; }
    ...
    internal sealed class __HACK__Stl1300Workaround
    {
        ...
        private __HACK__Stl1300Workaround()
        {
            try {
                ...
                try {
                    ...
                    if (newestInstance is null) {
                        Diagnostics = Diagnostics.Add(new TranslationDiagnostic(Severity.Warning, $"{DiagnosticPrefix}No instances of Visual Studio were found."));
                        return;
                    }
                ...
                {
                    Diagnostics = Diagnostics.Add(new TranslationDiagnostic(Severity.Warning, $"{DiagnosticPrefix}Could not locate the Visual Studio setup configuration COM service. {ex}"));
                    return;
                }
                catch (Exception ex) when (!Debugger.IsAttached) {
                    Diagnostics = Diagnostics.Add(new TranslationDiagnostic(Severity.Warning, $"{DiagnosticPrefix}An exception ocurred while attempting to locate Visual Studio installs: {ex}"));
                    return;
                }
                ...
                if (!File.Exists(defaultToolsVersionPath)) {
                    Diagnostics = Diagnostics.Add(new TranslationDiagnostic(Severity.Warning, $"{DiagnosticPrefix}Could not find default VC tools version for {visualStudioDescription}"));
                    return;
                }
                ...
                if (!File.Exists(intrin0Path)) {
                    Diagnostics = Diagnostics.Add(new TranslationDiagnostic(Severity.Warning, $"{DiagnosticPrefix}Could not locate intrin0.h at expected path '{intrin0Path}'"));
                    return;
                }
                ...
                if (patches.Count > 0) {
                    Diagnostics = Diagnostics.Add(new TranslationDiagnostic(Severity.Warning, $"{DiagnosticPrefix}Failed to apply the patch automatically. (Was intrin0.h modified by hand?)"));
                    return;
                }
                ...
                catch { Debug.WriteLine($"{DebugPrefix}Failed to write patched file to '{Path.GetFullPath(patchedFileName)}'"); }
            ...
            {
                Diagnostics = Diagnostics.Add(new TranslationDiagnostic(Severity.Warning, $"{DiagnosticPrefix}An exception ocurred while applying the workaround: {ex}"));
